...
def switchpage(pagenum):
    [frame.place_forget() for frame in frames]
    pageLbl.config(text=pages[pagenum])
...
frames = [tk.Frame(root, bg=getcolour()) for page in pages]
...
[subfilemenu.add_command(label=page, command=lambda p=idx:switchpage(p)) for idx, page in enumerate(pages)] # Pages from loop
